...
public class WorldSetEndPortal extends Command {
	...
	public void execute() {
		...
		if (this.handleWorld()) {
			if (dest.isEmpty()) {
				WorldConfig.get(worldname).defaultEndPortal = null;
				message(ChatColor.GREEN + "Default ender portal destination of world '" + worldname + "' cleared!");
			} else if (Portal.getPortalLocation(dest, null) != null) {
				WorldConfig.get(worldname).defaultEndPortal = dest;
